...
public class VistasVentas {
    ...
    private EntradaVistaDTO entrada(Entrada e) {
        return new EntradaVistaDTO(e.asientoId(), e.codigoAsiento(), e.tarifa().name(), e.precio());
    }

    public PagoVistaDTO pago(Pago p) {
        return new PagoVistaDTO(p.getId(), p.getReservaId(), p.getSubtotal(), p.getPromocionId(),
