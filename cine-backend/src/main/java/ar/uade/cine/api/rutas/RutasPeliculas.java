...
class RutasPeliculas {
    ...
    static void registrar(Javalin app, GestorCartelera cartelera,
                          GestorRevisionCartelera revision, GestorFunciones funciones,
        ...
        app.get("/api/peliculas/pendientes", ctx ->
                ctx.json(revision.listarPendientes().stream().map(vistas::pelicula).toList()));
        ...
        app.post("/api/peliculas/importadas", ctx -> {
            PedidoPeliculaDTO pedido = ctx.bodyAsClass(PedidoPeliculaDTO.class);
            ctx.status(HttpStatus.CREATED).json(vistas.pelicula(revision.importar(datosDe(pedido))));
        });
        ...
        app.post("/api/peliculas/{id}/confirmacion", ctx -> {
            int id = Parseo.id(ctx);
            buscar(cartelera, id);
            ctx.json(vistas.pelicula(revision.confirmar(id)));
        });

        app.post("/api/peliculas/{id}/descarte", ctx -> {
            int id = Parseo.id(ctx);
            buscar(cartelera, id);
            ctx.json(vistas.pelicula(revision.descartar(id)));
        });
