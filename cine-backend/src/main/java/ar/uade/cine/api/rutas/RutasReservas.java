...
import ar.uade.cine.servicio.ventas.CriteriosReserva;
        ...
        app.get("/api/reservas", ctx -> {
            ...
            List<Reserva> lista = email == null || email.isBlank()
                    ...
                    : clientes.buscarPorEmail(email.trim())
                            ...
                            .orElse(List.of());
            // vistas.reservas() y no un map de vistas.reserva(): el segundo pide la
            // funcion, la sala, la pelicula, el cliente y el pago de cada fila por
            // separado, o sea cinco consultas por reserva listada.
            ctx.json(vistas.reservas(lista.stream()
