...
import ar.uade.cine.api.vistas.VistasVentas;
...
class RutasPagos {
    ...
    static void registrar(Javalin app, GestorPagos pagos, GestorReservas reservas,
                          GestorCaja caja, VistasVentas vistas) {
        ...
        app.post("/api/reservas/{id}/checkout", ctx -> {
            ...
            ctx.status(HttpStatus.CREATED).json(new CheckoutVistaDTO(checkout.id(),
                    checkout.reservaId(), checkout.medio().name(), checkout.monto().aPesos(),
        ...
        app.get("/api/arqueo", ctx -> {
            Arqueo arqueo = caja.arqueoDe(Parseo.dia(ctx.queryParam("fecha"), "la fecha"));
            ctx.json(new ArqueoVistaDTO(arqueo.fecha().toString(), arqueo.total().aPesos(), arqueo.entradas(),
    ...
    private static Map<String, TotalMedioDTO> porMedio(Arqueo arqueo) {
        ...
        arqueo.porMedio().forEach((medio, acumulado) ->
                resumen.put(medio.name(), new TotalMedioDTO(acumulado.cantidad(), acumulado.total().aPesos())));
        return resumen;
    }
}
