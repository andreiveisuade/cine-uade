...
class RutasInformes {
    ...
    private static BorderoVistaDTO vista(Bordero bordero) {
        ...
        bordero.porTarifa().forEach((tarifa, total) ->
                porTarifa.put(tarifa.name(), new TotalTarifaDTO(total.cantidad(), total.total())));
        ...
        return new BorderoVistaDTO(bordero.funcionId(), bordero.pelicula(), bordero.sala(),
                ...
                bordero.espectadores(), bordero.recaudacionBruta(), bordero.descuentos(),
                bordero.recaudacionNeta(), porTarifa);
    }
}
