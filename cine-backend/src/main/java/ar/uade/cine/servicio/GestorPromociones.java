...
public class GestorPromociones {
    ...
    public Promocion crearMontoFijo(String nombre, double monto, LocalDate desde, LocalDate hasta,
                                    Set<DayOfWeek> dias, LocalTime horaDesde, LocalTime horaHasta,
                                    Set<MedioPago> medios) {
        if (monto <= 0) {
    ...
    public double descuentoDe(Promocion promocion, List<Entrada> entradas) {
