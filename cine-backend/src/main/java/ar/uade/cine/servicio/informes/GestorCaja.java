...
public class GestorCaja {
    ...
    public GestorCaja(PagoDAO pagoDAO, ReservaDAO reservaDAO, CompraCandyDAO compraCandyDAO) {
    ...
    public Dinero totalCobrado(LocalDate fecha) {
        return Dinero.sumar(pagoDAO.listarPorFecha(fecha).stream().map(Pago::getMonto).toList());
    }
    ...
    public Dinero totalCandyDe(LocalDate fecha) {
        return Dinero.sumar(compraCandyDAO.listarPorFecha(fecha).stream()
