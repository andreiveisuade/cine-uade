...
export async function vistaPelicula(contenedor, id) {
  ...
  const dias = porDia(funciones).map(([, delDia]) => `
    <section class="mb-5">
      ...
      <div class="grid gap-2 sm:grid-cols-2">
        ${delDia.map((f) => `
          <a href="#/funcion/${f.id}"
             class="flex items-center justify-between rounded border border-slate-300 bg-white px-3 py-2 hover:border-slate-500 dark:border-slate-700 dark:bg-slate-900 dark:hover:border-slate-500">
            <div>
              ...
              <div class="mt-1 flex gap-1">
                ${chip(etiqueta(f.proyeccion), "bg-indigo-100 text-indigo-800 dark:bg-indigo-900/40 dark:text-indigo-300")}
                ${chip(etiqueta(f.idioma), "bg-amber-100 text-amber-800 dark:bg-amber-900/40 dark:text-amber-300")}
              </div>
            </div>
            <div class="text-right">
              <p class="text-xs text-slate-500 dark:text-slate-400">desde</p>
  ...
  contenedor.innerHTML = `
    <a href="#/" class="text-sm text-slate-500 hover:text-slate-900 dark:text-slate-400 dark:hover:text-slate-100">&larr; Cartelera</a>
    <div class="mt-2 mb-6 flex flex-wrap gap-4">
      ...
      <div class="min-w-64 flex-1">
        ...
        ${pelicula.director
          ? `<p class="mt-1 text-sm text-slate-600 dark:text-slate-300">Dirección: ${escapar(pelicula.director)}</p>`
          : ""}
        ...
        ${pelicula.sinopsis
          ? `<p class="mt-3 max-w-prose text-sm text-slate-700 dark:text-slate-300">${escapar(pelicula.sinopsis)}</p>`
          : ""}
      </div>
    </div>
    <h2 class="mb-3 text-lg font-semibold">Funciones</h2>
    ${funciones.length ? dias : '<p class="text-slate-500 dark:text-slate-400">No hay funciones programadas.</p>'}
  `;
}
